...
import com.restaurant.reservation.dto.request.CheckAvailabilityRequest;
import com.restaurant.reservation.dto.response.ReservationResponse;
import com.restaurant.reservation.dto.response.CheckAvailabilityResponse;
...
import com.restaurant.reservation.strategyPattern.TableAssignmentContext;
...
public class ReservationService {
    ...
    private final TableAssignmentContext tableAssignmentContext;
    ...
    public ReservationResponse create(CreateReservationRequest req, String customerEmail) {
        ...
        //  Fetch all unreserved tables for this slot
        ...
        // Filter to only tables that can fit the guest count
        List<RestaurantTable> freeTables = availableTables.stream()
                .filter(table -> table.getCapacity() >= req.getGuestCount())
        ...
        //  Use Strategy Context to assign the best table based on restaurant config
        RestaurantTable assignedTable = tableAssignmentContext.assignTable(
                restaurant, 
                freeTables, 
                req.getGuestCount()
        );
    ...
    // ================= CHECK AVAILABILITY & ALLOCATE TABLE =================
    public CheckAvailabilityResponse checkAvailabilityAndAssign(CheckAvailabilityRequest req) {
        LocalDate date = LocalDate.parse(req.getReservationDate());
        ...
        List<RestaurantTable> fitTables = availableTables.stream()
                .filter(table -> table.getCapacity() >= req.getGuestCount())
                .collect(Collectors.toList());
        ...
        if (fitTables.isEmpty()) {
            return CheckAvailabilityResponse.builder()
                    .isAvailable(false)
                    .message("No available tables for " + req.getGuestCount() + " guests at this time slot.")
                    .build();
        }
        ...
        try {
            ...
            return CheckAvailabilityResponse.builder()
                    .isAvailable(true)
                    .tableId(assignedTable.getId())
                    .tableNumber(assignedTable.getTableNumber())
                    .location(assignedTable.getLocation().name())
                    .message("Table assigned successfully using " + restaurant.getAssignmentStrategy() + " strategy.")
                    .build();
        } catch (Exception e) {
            return CheckAvailabilityResponse.builder()
                    .isAvailable(false)
                    .message("Could not assign table: " + e.getMessage())
                    .build();
        }
    }
}