...
public class RestaurantService {
    ...
    public RestaurantResponse uploadImage(Long restaurantId, MultipartFile imageFile, String userEmail) throws IOException {
        ...
        if (!isOwner && !isAdmin) {
            throw new RuntimeException("You can only upload images for restaurants , must be an admin");
        }
    ...
    public RestaurantResponse update(Long restaurantId, CreateRestaurantRequest req, String adminEmail) {
        ...
        // Allow if user is the owner OR if user has the global ADMIN role
        User user = userRepository.findByEmail(adminEmail)
                .orElseThrow(() -> new RuntimeException("User not found"));

        boolean isOwner = restaurant.getAdmin().getEmail().equals(adminEmail);
        boolean isAdmin = user.getRole() == Role.ADMIN;
    ...
    private RestaurantResponse mapToResponse(Restaurant r) {
        ...
        // Convert legacy absolute OS paths to relative web paths on-the-fly
        if (imageUrl != null && !imageUrl.isEmpty()) {
            // Normalize backslashes
            imageUrl = imageUrl.replace("\\", "/");
            // If it still looks like an absolute path (e.g. D:/Reservation/uploads/restaurants/xyz.png)
            // extract just the relative part
            int uploadsIdx = imageUrl.indexOf("uploads/restaurants/");
            if (uploadsIdx > 0) {
                imageUrl = imageUrl.substring(uploadsIdx); // -> uploads/restaurants/xyz.png
            }
        }
